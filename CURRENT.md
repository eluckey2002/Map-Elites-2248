...
The implementation check now distinguishes an open 2x2 real play from its one-stone twin (`0` versus `1/3`) and pins a non-power-of-two scale fixture, but this is still only a mechanized candidate descriptor—not an accepted MAP-Elites axis. No evidence yet shows useful range across existing levels or policies, separation of play styles, prediction of fun, or fitness value. The next bounded step is a cheap range probe on existing deterministic games; do not start a large experiment or repeat RESULT-0024 with more seeds.
