...
const REQUIREMENTS = Object.freeze({
  ...
  P5: 'SUPPORTED when every percentile remains in its modal or adjacent greed bin on >=80% of exact-complete games; FALSIFIED below 60%',
});
...
function greedBinStability(rows, percentile) {
  ...
  if (!bins.length) return { modalGreedBin: null, sameOrAdjacentGreedRate: null };
  ...
  return {
    modalGreedBin,
    sameOrAdjacentGreedRate: bins.filter((bin) => Math.abs(bin - modalGreedBin) <= 1).length / bins.length,
  };
}

function summarizeDeterministicGreed(rows, { expectedGames = 128 } = {}) {
  ...
  const greedMeanRange = Number.isFinite(greedMeans[0])
    ...
    : null;
  const minimumGreedBinStability = Math.min(...policies.map(({ sameOrAdjacentGreedRate }) => (
    Number.isFinite(sameOrAdjacentGreedRate) ? sameOrAdjacentGreedRate : -Infinity
  )));
  const p1 = classify(
  ...
  const p5 = classify(
    minimumGreedBinStability >= 0.80,
    minimumGreedBinStability < 0.60,
  );
...
module.exports = { REQUIREMENTS, classify, summarizeDeterministicGreed };
