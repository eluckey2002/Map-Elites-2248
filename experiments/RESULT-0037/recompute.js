...
function reduceCorpus(corpus) {
  ...
  const policies = percentiles.map((percentile) => {
    ...
    const bins = exactGames.map(({ cell }) => cell.greed);
    ...
    return {
      ...
      meanExactGreed: exactGames.length
        ...
        : null,
      modalGreedBin,
      sameOrAdjacentGreedRate: bins.length
        ? bins.filter((bin) => Math.abs(bin - modalGreedBin) <= 1).length / bins.length
        : null,
    };
  });
  const cells = new Map();
  ...
  const p5 = classify(minimumGreedBinStability >= 0.80, minimumGreedBinStability < 0.60);
