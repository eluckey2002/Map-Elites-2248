...
const { CELL_NAMES, playPercentile } = require('../../solver/greed-descriptor-screen');
...
function buildCorpus({
  ...
  for (const percentile of percentiles) {
    for (const level of levels) {
      for (const seed of seeds) {
        rows.push(playPercentile(level, seed, percentile, {
          denominator: 'exact',
  ...
  return {
    ...
    proofStanding: 'exact per-move denominator when child enumeration completes; UNKNOWN games on any timeout; heuristic observation over the fixed policy/level/seed panel',
    definitions: {
      halfScoreMove: 'first move reaching half the final score, divided by moves used',
      ...
      cells: CELL_NAMES,
      exclusions: ['human difficulty', 'fun', 'player preference', 'MAP-Elites fitness', 'build potential', 'levels outside the fixed panel', 'exact greed for timed-out games'],
    },
    panel: {
    ...
    rows,
    decision: summarizeRegistered(rows, { expectedGames: EXPECTED_GAMES }),
  };
}
...
function main(argv = process.argv.slice(2)) {
  ...
  if (!output) throw new Error('usage: run.js --protocol RESULT-0036 --out <path>');
  ...
  const body = buildCorpus({
    onCell: ({ percentile, level, completedGames }) => {
      process.stderr.write(`RESULT-0036 ${percentile} level ${level}: ${completedGames}/${EXPECTED_GAMES}\n`);
    },
  });
