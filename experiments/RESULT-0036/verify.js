...
function verifyRegistration(artifact) {
  ...
  if (!registration || registration.exploratory !== false || registration.protocol !== RESULT) {
    throw new Error('artifact is not registered reportable RESULT-0036 evidence');
  }
...
function verifyRow(row) {
  ...
  if (canonicalJson(descriptorCell(row.descriptors)) !== canonicalJson(row.cell)) {
    throw new Error('descriptor cell recomputation mismatch');
  }
  return true;
}
...
function verifyArtifact(artifact) {
  ...
  const decision = summarizeRegistered(artifact.rows, { expectedGames: EXPECTED_GAMES });
