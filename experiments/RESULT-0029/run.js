...
function main(argv = process.argv.slice(2)) {
  const registration = requireProtocolOrExit(process.argv, { name: 'RESULT-0029 synthetic descriptor validation' });
  const kind = argv[0];
  if (!['controls', 'confirmation'].includes(kind)) {
    throw new Error('usage: run.js controls|confirmation --protocol RESULT-0029 --levels <csv> --seeds <csv> --out <path>');
  }
