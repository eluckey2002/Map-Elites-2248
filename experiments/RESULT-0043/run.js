...
function main(argv = process.argv.slice(2)) {
  ...
  if (!output) throw new Error('usage: run.js --protocol RESULT-0043 --out <path>');
  ...
  const body = buildCorpus({
    onCell: ({ percentile, level, completedGames }) => {
      process.stderr.write(`RESULT-0043 ${percentile} level ${level}: ${completedGames}/${EXPECTED_GAMES}\n`);
    },
  });
