...
const { choosePercentileCandidate, playPercentile } = require('../../solver/greed-descriptor-screen');
...
function qualify() {
  ...
  const watchdogCalibration = playPercentile(
    LEVELS.find(({ level }) => level === 10),
    33800004,
    1,
    {
      denominator: 'exact',
  ...
  assert.equal(watchdogCalibration.moves, 22);
  assert.equal(
    watchdogCalibration.denominatorObservations.some(({ reason }) => reason === 'timeout'),
    false,
  );
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'result-0043-qualification-'));
  try {
    ...
    const recomputation = spawnSync(process.execPath, [
      path.join(ROOT, 'experiments/RESULT-0043/recompute.js'), '--corpus', corpusPath,
    ...
    const contract = JSON.parse(fs.readFileSync(path.join(__dirname, 'closeout-contract.json'), 'utf8'));
    const contractArgs = [...contract.recomputation.argv];
    const corpusIndex = contractArgs.indexOf('--corpus');
    assert.notEqual(corpusIndex, -1);
    contractArgs[corpusIndex + 1] = corpusPath;
    const contractRecomputation = spawnSync(contractArgs[0], contractArgs.slice(1), {
      cwd: path.resolve(__dirname, contract.recomputation.cwd),
      encoding: 'utf8',
    });
    assert.equal(contractRecomputation.status, contract.recomputation.expected_exit_code, contractRecomputation.stderr);
    assert.equal(contractRecomputation.stdout, `${JSON.stringify(reduceCorpus(baseCorpus))}\n`);
  } finally {
  ...
  return {
    ...
    controls: {
      ...
      watchdogCalibration: {
        ...
        level: 10,
        percentile: 1,
        moves: watchdogCalibration.moves,
        workLimitedMoves: watchdogCalibration.denominatorObservations
          .filter(({ reason }) => reason === 'work-limit').length,
        timeoutMoves: 0,
      },
      ...
      contractRecomputationPath: 'PASS',
