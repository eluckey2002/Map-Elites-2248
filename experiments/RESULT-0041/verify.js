...
function verifyRegistration(artifact) {
  ...
  if (!registration || registration.exploratory !== false || registration.protocol !== RESULT) {
    throw new Error('artifact is not registered reportable RESULT-0041 evidence');
  }
...
function verifyRow(row) {
  ...
  for (const observation of row.denominatorObservations) {
    ...
    if (observation.standing === 'UNKNOWN' && observation.reason === 'work-limit'
      && observation.visitedPathStates !== EXACT_MAX_PATH_STATES) {
      throw new Error('work-limit state count mismatch');
    }
...
function verifyArtifact(artifact) {
  ...
  if (canonicalJson(artifact.panel.seeds) !== canonicalJson(CONFIRMATION_SEEDS)) {
    throw new Error('registered seed panel mismatch');
  }
  verifyMatrix(artifact.rows, CONFIRMATION_SEEDS);
  for (const row of artifact.rows) verifyRow(row);
  const decision = summarizeDeterministicGreed(artifact.rows, {
