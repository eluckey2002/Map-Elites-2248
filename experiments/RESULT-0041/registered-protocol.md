# Frozen protocol — real-board greed-ratio validation with bound receipts
...
It supersedes RESULT-0040, whose first qualification attempt stopped before
confirmation because its independent reducer did not exclude a null policy
mean from the policy-level correlation.
...
30,000 ms child-process watchdog is infrastructure protection, and any timeout
invalidates the run.
...
33,800,000–33,800,007: 128 ordered games, percentile then level then seed.
These are real 5×8, 5×7, 6×5, and 4×8 shipped boards, not micro-boards.
...
panel, not an individual seed. The production verification seam is
`experiments/RESULT-0041/verify.js#verifyArtifact`.
...
- **C1 — candidate-measure, arithmetic baseline.** Production trace fixtures
  ...
  Failure means qualification `FAIL` and confirmation does not start. Evidence:
  the manifest-derived qualification test receipt.
- **C2 — simulation-policy, deterministic exact/reference control.** An exact
  fixture must return its known maximum; repeated capped results must be
  byte-identical; proxy and exact modes must produce the same score, win, move
  count, and terminal reason on burned seed 33,300,000. Failure stops
  confirmation. Evidence: qualification tests through the real play seam.
- **C3 — candidate-measure, positive and negative manipulation.** The planted
  ...
  stops confirmation. Evidence: `qualification.json`.
- **C4 — mutation-qualification, known kills.** The exact production verifier
  ...
  makes qualification `FAIL`. Evidence: every mutant and disposition in
  `qualification.json`.
- **C5 — candidate-measure, independent analysis.** Registered analysis and
  ...
  Failure stops confirmation. Evidence: qualification receipt and executable
  stdout recomputation.
- **C6 — candidate-measure, orthogonal manipulation.** Changing only
  ...
  Failure stops confirmation. Evidence: `qualification.json`.
- **C7 — common repository baseline.** The solver suite must retain exactly
  ...
  stops confirmation. Evidence: baseline command output and qualification
  receipt.
...
- **P5 non-vacuous stability:** for each policy, calculate the share of exact
  ...
  0.45..<0.75`, `high >=0.75`). `SUPPORTED` when every policy is at least
  80%; `FALSIFIED` when any is below 60%; otherwise `INCONCLUSIVE`. This retains
  the project's pre-existing 80% floor but removes adjacency, so a middle-bin
  policy can fail.
...
before qualification. Qualification may use only synthetic fixtures and
already-burned seed 33,300,000. It has one attempt identity; three failures at
the same step end `UNVERIFIED` rather than trigger redesign in place.
