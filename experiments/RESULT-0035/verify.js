...
function verifyRegistration(artifact) {
  ...
  if (!registration || registration.exploratory !== false || registration.protocol !== RESULT) {
    throw new Error('artifact is not registered reportable RESULT-0035 evidence');
  }
...
function verifyArtifact(artifact) {
  ...
  const summary = summarizeMap(artifact.rows, { spreadCut: SPREAD_CUT, capacity: CELL_CAPACITY });
  if (canonicalJson(summary) !== canonicalJson(artifact.decision)) {
