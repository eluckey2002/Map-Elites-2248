...
function pearson(xs, ys) {
  ...
  const usable = xs.map((x, index) => [x, ys[index]])
    .filter(([x, y]) => Number.isFinite(x) && Number.isFinite(y));
  if (usable.length < 2) return null;
  const usableXs = usable.map(([x]) => x);
  const usableYs = usable.map(([, y]) => y);
  const xMean = mean(usableXs);
  const yMean = mean(usableYs);
  const numerator = usableXs.reduce((sum, value, index) => sum + (value - xMean) * (usableYs[index] - yMean), 0);
  const xDenominator = Math.sqrt(usableXs.reduce((sum, value) => sum + (value - xMean) ** 2, 0));
  const yDenominator = Math.sqrt(usableYs.reduce((sum, value) => sum + (value - yMean) ** 2, 0));
