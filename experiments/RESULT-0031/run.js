...
function findMinimumChain(state, chain) {
  if (chain.length >= state.minChain) return chain;
  ...
  for (let dy = -1; dy <= 1; dy++) {
    for (let dx = -1; dx <= 1; dx++) {
      ...
      const found = findMinimumChain(state, [...chain, neighbor]);
...
function firstValidMove(state) {
  for (const row of state.grid) {
    for (const tile of row) {
      if (!tile || isBlockedTile(tile)) continue;
      const found = findMinimumChain(state, [tile]);
      if (found) return found;
    }
  }
  return null;
...
function candidatePool(state) {
  ...
  if (paths.length === 0) {
    const fallback = firstValidMove(state);
    if (fallback) paths.push({ chain: fallback });
  }
  const seen = new Set();
  const candidates = [];
...
function runCells(levelNumbers, seeds, subjects = SUBJECTS, play = playSynthetic) {
      ...
      for (const seed of seeds) {
        try {
          cells.push({
            ...
            level: level.level,
            seed,
            ...play(level, seed, subject),
          });
        } catch (error) {
          throw new Error(`subject ${subject.id} level ${level.level} seed ${seed}: ${error.message}`, {
            cause: error,
          });
        }
      }
...
function main(argv = process.argv.slice(2)) {
  const registration = requireProtocolOrExit(process.argv, { name: 'RESULT-0031 synthetic descriptor validation' });
  const kind = argv[0];
  if (!['controls', 'confirmation'].includes(kind)) {
    throw new Error('usage: run.js controls|confirmation --protocol RESULT-0031 --levels <csv> --seeds <csv> --out <path>');
  }
