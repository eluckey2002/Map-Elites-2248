...
- **`node --test solver/tests/*.test.js` reports 390 tests: 385 pass, four fail deliberately, and one is skipped.** The four failures are two stale candidate receipts, a generated-view staleness check, and a date-drift check. One of them carries its own "THIS FAILURE IS KNOWN AND DECIDED, it is not yours to fix" message. Do not clear them by re-authoring, archiving, or exempting.
