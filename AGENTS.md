...
- **`node --test solver/tests/*.test.js` reports 422 tests: 419 pass, two fail deliberately, and one is skipped.** The two failures are the stale candidate receipts for levels 52 and 54. One carries its own "THIS FAILURE IS KNOWN AND DECIDED, it is not yours to fix" message. Do not clear them by re-authoring, archiving, or exempting.
- **The Universe Map expires 7 days after `universe/contract.json`'s `asOf`.** When `verify-universe-map.js` reports it stale or drifted, set `asOf` to today, run `node tools/build-universe-map.js`, and review the diff: it should only move counts and hashes that the ledger, `CURRENT.md`, or `src/game.js` actually changed.
