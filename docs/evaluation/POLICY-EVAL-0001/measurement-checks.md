...
- **Crafted-bypass test:** `solver/tests/policyBenchmark.test.js`, `a forged candidate receipt key does not resolve content under that identity` and `exact candidate content with an absent or forged receipt self-identity is rejected`; the latter preserves correct candidate bytes while first removing both receipt self-identity fields and then forging the self-identity, requiring each exact rejection reason.
...
- **Crafted-bypass test:** `solver/tests/policyBenchmark.test.js`, altered-coordinate, false-win/premature-loss/post-terminal, missing-trace, seed/subject mismatch, bomb/target/B=H precedence, illegal choice, thrown-harness, RNG-consumption, and no-choice-with-legal-move cases; `solver/tests/humanBenchmark.test.js`, the collect-level reference- and diagnostic-fault controls inject each unresolved result separately through the public collector and renderer. Each requires 15 unresolved files, no admitted rows, unavailable ranking/score/regression data, explicit UNRESOLVED text, and no undefined or zero-regression rendering. Both widened controls failed on the actual pre-repair collector/renderer before repair.
...
- **Crafted-bypass test:** `solver/tests/humanBenchmark.test.js`, `missing required and actual extra files stay visible through collect and render` stages the real corpus with one required receipt-bound recording absent and an actual extra file present, then requires a 12-file denominator, 11 available attempts, an unresolved full panel, a labeled resolved subset, and both filenames in public text output; both runtime-fault controls now traverse renderText and require unavailable (not undefined or zero) regression counts in both empty unresolved panels; `--json flushes one complete JSON document larger than the default pipe buffer` protects complete machine-readable output.
...
- **Enforcement:** HARD for deterministic Step 2 descriptive output; no generalized or promotion claim is admitted. The 2026-09-05 widening was first exercised as a local report-only probe: both controls went red on the real defective path. Blocking test integration requires the repaired bad twins and unchanged real corpus to pass, plus independent acceptance; it does not alter an experiment-admission gate.
