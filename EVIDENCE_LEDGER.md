...
As of 2026-09-05, `solver/bot.js`'s `rolloutValue` was fixed to use the bot's own shipped `tieBreak`/`pathWidth` search settings instead of always falling back to the weakest configuration; the bug changed which move the bot played on roughly 30% of a real self-play sample. This is the same `chooseBaseMove` that both arms of `RESULT-0018`/`RESULT-0020` compare, so their **1.271 moves saved** figure now describes pre-fix behavior, not the current bot: on the exact Level 51 seed 1 board those tests exercise, the baseline chooser alone now reaches the target in 13 moves post-fix, matching the target-aware challenger exactly, where it previously took 17 (`solver/tests/targetAwareChallenger.test.js`). A diagnostic (not pre-registered) comparison across 14 levels spanning the difficulty curve, 40 seeds each, found no win-rate regression (two levels moved 97.5% -> 100%, none moved down) and score deltas within ±4.3%, consistent with seed noise rather than a systematic shift — `DECISION-0003`'s level curve is not disturbed by this fix on this sample. Not re-measured: whether `RESULT-0018`/`RESULT-0020`'s relative lift still holds at a similar magnitude, and whether the bot's regression on the frozen Level 26 hard case (bot playing *worse* than the pre-fix bot despite general improvement, found the same day) changes again now that `rolloutValue` has also been fixed. Neither `RESULT-0026`'s FALSIFIED verdict nor level-authoring evidence is affected: the exact-sum policy's failure is a distinct mechanism (a rigid fallback DFS, not the rollout estimate), and `calib-1` is deliberately decoupled from live `bot.js` changes (`RESULT-0027`).
...
- **proof_class:** `direct_source` — the counts, identities, and recorded lifts are what the archive file literally contains. No generalizing claim is made; there was no preregistered protocol.
- **as_of:** 2026-08-28
...
- **proof_class:** `direct_source` — axes equality, seed disjointness, cell counts, and recorded lifts are read directly from the two archive files. No generalizing claim is made; there was no preregistered protocol.
- **as_of:** 2026-08-28
