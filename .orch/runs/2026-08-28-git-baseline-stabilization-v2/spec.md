...
acceptance:
  ...
  - id: A4
    ...
    oracle_class: deterministic
  - id: A5
    criterion: The candidate canonical revision is no worse than the frozen unsandboxed solver baseline—198 tests, 195 pass, and exactly the three deliberate stale-receipt failures for `candidate-levels-52.json`, `candidate-levels-54.json`, and `candidate-levels.json`—and also passes the curve health gate, the MAP-Elites verifier against the retained independent-round artifact, and `git diff --check`.
    oracle: unsandboxed `node --test solver/tests/*.test.js` with exact failure-identity comparison; `node solver/verify-loop.js`; `node solver/verify-map-elites.js .orch/runs/2026-08-28-map-elites-independent-round/evidence`; `git diff --check <integration-range>`
    oracle_class: deterministic
  - id: A6
...
evidence:
  - `.orch/runs/2026-08-28-git-baseline-stabilization/worklog.md` failed predecessor and reusable preservation evidence
  - `.orch/runs/2026-08-28-git-baseline-stabilization/evidence/recovery-manifest.md`
